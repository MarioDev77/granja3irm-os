...
const COLUMN_BY_FIELD = {
  ...
  address: 'address', products: 'products', notes: 'notes',
};
...
export async function PATCH(request, { params: __p }) {
  ...
  const { rows: existingRows } = await query('SELECT * FROM suppliers WHERE id = $1', [params.id]);
  ...
  await query(
    ...
    [genId(), session.user.id, supplier.id, JSON.stringify(parsed.data), `${session.user.name} atualizou o fornecedor ${supplier.name}.`]
  );
